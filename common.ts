...
import { readableStreamToString, JinaAiError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new JinaAiError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new JinaAiError(e, e.response?.data)
                }
                if (innerError instanceof JinaAiError) {
                    // Got "JinaAiError" from the above try block
                    throw innerError;
                }
